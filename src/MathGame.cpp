...
int main() {
  ...
  ArithmeticProblem ap[a]; // Initializing all ArithmeticProblems
  for (int i = 0; i < a; i++) {
    srand(seed + i * 2 + 2); // Use a different seed each time
    ap[i] = ArithmeticProblem(); // Then make new ArithmeticProblem
  }

  GeometryProblem gp[g]; // Initializing all GeometryProblems
  for (int i = 0; i < g; i++) {
    srand(seed + i * 7 + 7); // Use a different seed each time
